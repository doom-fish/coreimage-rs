...
- **Breaking:** removed the diagnostic passthrough processor `CIImageProcessor` (`apply_passthrough`, `last_invocation`, `last_invocation_json`) together with its process-wide invocation snapshot types `CIImageProcessorInput`, `CIImageProcessorOutput` and `CIImageProcessorInvocation`. It was a hard-coded test kernel that recorded every call in one global snapshot; write processors with `CIImageProcessorKernel`.
